...
class EvalAssistant:
    ...
    def create_file(self, file_list: t.Union[list]):
        files_id = []
        ...
        for file in file_list:
            try:
                ...
                file = self.client.files.create(
                    file=file_location, purpose="assistants"
                )
            ...
            evaluator_response = (
                self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": ASSISTANT_EVALUATOR_STARTER_PROMPT.format(
            ...
            evaluator_response = (
                self.client.chat.completions.create(
                    ...
                    messages=[
                        {
                            "role": "system",
                            "content": ASSISTANT_EVALUATOR_PROMPT.format(
                                evaluator_persona=evaluator_persona,
    ...
    def simulate_conversation(
        ...
        for index, evaluator_persona in enumerate(evaluator_persona_list):
            ...
            while i <= trial_count:
                ...
                conversation.append({"role": "user", "content": evaluator_response})
                ...
                while run.status in ["queued", "in_progress", "cancelling"]:
                    ...
                    run = self.client.beta.threads.runs.retrieve(
                        thread_id=user_bot_thread_id, run_id=run.id
                    )
    ...
    def evaluate(self, data: t.Union[list], checks: t.Union[list]):
        ...
        for index_outer in range(len(data2)):
            ...
            for index_inner in range(len(data2[index_outer]["conversation"])):
                if (
                    data2[index_outer]["conversation"][index_inner]["role"] == "user"
                ) and (
                    index_inner + 1 in range(len(data2[index_outer]["conversation"]))
                ):
                    formatted_conversation = [
                        data2[index_outer]["conversation"][index_inner],
                        data2[index_outer]["conversation"][index_inner + 1],
                    ]

                    formatted_conversation[0]["role"] = data2[index_outer][
                        "conversation"
                    ][index_inner]["role"]
                    formatted_conversation[0]["content"] = data2[index_outer][
                        "conversation"
                    ][index_inner]["content"]
                    try:
                        formatted_conversation[0]["rewritten_query"] = data2[
                            index_outer
                        ]["conversation"][index_inner]["rewritten_query"]
                    except:
                        pass
                    formatted_conversation[1]["role"] = data2[index_outer][
                        "conversation"
                    ][index_inner + 1]["role"]
                    formatted_conversation[1]["content"] = data2[index_outer][
                        "conversation"
                    ][index_inner + 1]["content"]
            ...
            res = self.eval_llm.evaluate(data=eval_input, checks=check2)
            res = [
                {
                    k: v
                    for k, v in item.items()
                    if k not in ["question", "context", "response"]
                }
                for item in res
            ]
            ...
            res = self.eval_llm.evaluate(
                ...
                checks=[
                    ConversationSatisfaction(
                        user_persona=user_persona, llm_persona=llm_persona
                    )
                ],
            )
            for index in range(len(data2)):
                data2[index]["score_conversation_satisfaction"] = res[index][
                    "score_conversation_satisfaction"
                ]
                data2[index]["explanation_conversation_satisfaction"] = res[index][
                    "explanation_conversation_satisfaction"
                ]
        return data2
