...
def parse_file(file_list: t.Union[list]):
    file_content = []
    for file in file_list:
        try:
            ...
            if file.split(".")[-1] == "docx":
                ...
                file_content.append({"content": content, "file_name": file})
...
def vector_search(
    file_list: t.Union[list], conversation: list[dict], openai_api_key: str
):
    context_chunk = []
    ...
    vectors = np.array(
        [list(embedding_list.embedding) for embedding_list in vectors_embeddings.data]
    ).astype(np.float32)
            ...
            if conversation_list[i]["role"] == "user":
                if i > 0:
                    ...
                    rewritten_query = (
                        QueryRewrite()
                        .setup(settings)
                        .run(pl.DataFrame(query_rewrite_input))["output"]
                        .to_dicts()[0]["revised_question"]
                    )
                ...
                search_vector = [
                    embedding.embedding for embedding in search_vector.data
                ]
                search_vector = np.array(
                    [list(embedding_list) for embedding_list in search_vector]
                ).astype(np.float32)
                ...
                results_context_df = pd.DataFrame(
                    {"distances": distances[0], "ann": ann[0]}
                )
                context_chunk_df_final = pd.merge(
                    results_context_df,
                    context_chunk_df,
                    left_on="ann",
                    right_index=True,
                ...
                context_shortlisted = context_chunk_df_final[
                    context_chunk_df_final["distances"] < mean - threshold * std_dev
                ]
                if len(context_shortlisted) > 0:
                    ...
                    try:
                        context_shortlisted = context_chunk_df_final.sort_values(
                ...
                conversation_list_new.append(conversation_list[i])
            rewritten_query = None
